...
\vspace{0.5cm}
\noindent\textbf{Słowa kluczowe:} analiza skupień, klastry, ceny mieszkań, K-means, metoda hierarchiczna
...
W części analitycznej zastosowano dwie metody grupowania: algorytm K-średnich oraz hierarchiczną analizę skupień metodą Warda. Pozwoliło to nie tylko na wyznaczenie segmentów rynku, lecz także na porównanie wyników uzyskanych różnymi podejściami i ocenę ich spójności.
...
W niniejszym rozdziale zaprezentowano wyniki przeprowadzonej analizy skupień dla rynku nieruchomości w Krakowie. Proces badawczy obejmował wyznaczenie optymalnej liczby klastrów, podział zbioru danych algorytmem K-średnich oraz weryfikację struktury danych metodą hierarchiczną.
...
Po zastosowaniu algorytmu K-średnich dla $k=3$ na ustandaryzowanych danych, otrzymano podział zbioru na trzy grupy o zróżnicowanej charakterystyce. W tabeli \ref{tab:klastry} zestawiono średnie wartości zmiennych dla każdego z segmentów (w oryginalnych jednostkach).
...
W celu potwierdzenia istnienia naturalnych skupisk w danych, przeprowadzono klastrowanie hierarchiczne metodą Warda na reprezentatywnej próbie obserwacji. Dendrogram (Rys. \ref{fig:dendrogram}) potwierdził zasadność podziału na trzy główne gałęzie, co jest spójne z wynikami metody K-średnich.
...
Dokonano porównania wyników uzyskanych dwiema niezależnymi metodami: algorytmem K-średnich oraz metodą hierarchiczną Warda. 
...
\textbf{Po pierwsze, w danych można wskazać wyraźne skupienia ofert mieszkań.} Zastosowanie metody łokcia w analizie algorytmu K-średnich wskazało, że optymalnym rozwiązaniem jest podział zbioru na trzy klastry. Wynik ten został dodatkowo potwierdzony przez analizę hierarchiczną metodą Warda, której dendrogram również sugerował istnienie trzech głównych grup. Oznacza to, że badany zbiór nie ma charakteru przypadkowego ani jednorodnego, lecz wykazuje wewnętrzną strukturę, którą można opisać za pomocą metod grupowania.
